...
void main() {
  ...
  group('NotificationsCubit', () {
    ...
    test('loadPreferences emits Loading then Loaded with defaults', () async {
      ...
      expect(loaded.preferences.salawatIntervalHours, 1);
    ...
    test('setSalawatInterval persists interval', () async {
      final cubit = buildCubit();
      await cubit.loadPreferences();

      await cubit.setSalawatInterval(2);

      final loaded = cubit.state as NotificationsLoaded;
      expect(loaded.preferences.salawatIntervalHours, 2);
      expect(storage.getInt(AppConstants.salawatIntervalHoursPrefKey), 2);
